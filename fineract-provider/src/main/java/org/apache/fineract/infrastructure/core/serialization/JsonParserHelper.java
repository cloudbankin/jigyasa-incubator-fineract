...
//    edited
    public Time extractLocalTime(final String parameterName, final JsonElement element,
            final Set<String> parametersPassedInCommand) {
        ...
        if (element.isJsonObject()) {
            ...
            final Locale clientApplicationLocale = extractLocaleParameter(object);
            value = extractLocalTime(parameterName, object, timeFormat, clientApplicationLocale, parametersPassedInCommand);
        }
        return value;
    }
...
// edited
    public Time extractLocalTime(final String parameterName, final JsonObject element, final String timeFormat,
            final Locale clientApplicationLocale, final Set<String> parametersPassedInCommand) {
        Time value = null;
        Time value1 = null;
        String timeValueAsString=null;
        if (element.isJsonObject()) {
            ...
            if (object.has(parameterName) && object.get(parameterName).isJsonPrimitive()) {
                ...
                try{
                    ...
                    final JsonPrimitive primitive = object.get(parameterName).getAsJsonPrimitive();
                     timeValueAsString = primitive.getAsString();
                     String namepass[] = timeValueAsString.split(":");
                     int hour = Integer.parseInt(namepass[0]); 
                     int minute = Integer.parseInt(namepass[1]); 
                     int second = Integer.parseInt(namepass[2]); 
                    if (StringUtils.isNotBlank(timeValueAsString)) {
                    	LocalTime valueTemp = LocalTime.of(hour, minute, second);
                    	value = Time.valueOf(timeValueAsString); 
//                    	value = valueTemp.plusSeconds(1);
//                    	value = ISODateTimeFormat.hourMinuteSecond().parseLocalTime(timeValueAsString) ;
//                        value = timeFormtter.print(valueTemp);
                    }    
                }
                catch(IllegalArgumentException e ){
                    final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
