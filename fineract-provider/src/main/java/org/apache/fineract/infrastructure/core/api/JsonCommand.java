...
public final class JsonCommand {
    ...
    private boolean differenceExists(final Time time, final Time workingCopyValue) {
        boolean differenceExists = false;

        if (time != null) {
            differenceExists = !time.equals(workingCopyValue);
        } else {
            differenceExists = workingCopyValue != null;
        }
    ...
    public boolean isChangeInTimeParameterNamed(final String parameterName, final Time meetingtime,final String timeFormat) {
        Time time = null;
        if (meetingtime != null) {
            DateTimeFormatter timeFormtter = DateTimeFormat.forPattern(timeFormat);
            time = Time.valueOf(meetingtime.toString());
        }
        return isChangeInLocalTimeParameter(parameterName, time);
    }
    ...
    public boolean isChangeInLocalTimeParameter(final String parameterName, final Time time) {
        boolean isChanged = false;
        if (parameterExists(parameterName)) {
            final Time workingValue = localTimeValueOfParameter(parameterName);
            isChanged = differenceExists(time, workingValue);
        }
        return isChanged;
    }
    ...
    public Time localTimeValueOfParameter(final String parameterName) {
        return this.fromApiJsonHelper.extractLocalTime(parameterName, this.parsedCommand);
    }
