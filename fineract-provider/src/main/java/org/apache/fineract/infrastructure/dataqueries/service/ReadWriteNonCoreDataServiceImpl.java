...
public class ReadWriteNonCoreDataServiceImpl implements ReadWriteNonCoreDataService {
    ...
    private String validateColumn(final ResultsetColumnHeaderData columnHeader, final String pValue, final String dateFormat,
        ...
        if((!StringUtils.isAlphanumeric(paramValue)) && (columnHeader.getColumnName().equals("IFSC Code")))
        {
        	final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
            final ApiParameterError error = ApiParameterError.parameterError("error.msg.column.value.alphanumeric", "value must be alphanumeric",
                    columnHeader.getColumnName());
            dataValidationErrors.add(error);
            throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.",
                    dataValidationErrors);
        }
        if((columnHeader.getColumnName().equals("IFSC Code")) && (paramValue.length()!=11))
        {
        	final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
            final ApiParameterError error = ApiParameterError.parameterError("error.msg.column.value.have.11 digits", "value must have 11 characters",
                    columnHeader.getColumnName());
            dataValidationErrors.add(error);
            throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.",
                    dataValidationErrors);
        }
        if(columnHeader.getColumnName().equals("Account Number") && (duplicateAccountNumberheck(paramValue)==null))
        {
        	final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
            final ApiParameterError error = ApiParameterError.parameterError("error.msg.Account Number.value.already.exists", "Account Number already exists",
                    columnHeader.getColumnName());
            dataValidationErrors.add(error);
            throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.",
                    dataValidationErrors);
    ...
    public String duplicateAccountNumberheck(String paramValue)
    {
    	final String sql="select count(`Account Number`) from `Bank Detail` where `Account Number`='" + paramValue + "'";
        String result="";    
        final int rs = this.jdbcTemplate.queryForInt(sql);
