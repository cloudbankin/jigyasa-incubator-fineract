...
public class ScheduledJobRunnerServiceImpl implements ScheduledJobRunnerService {
    ...
    @Override
    @CronTarget(jobName = JobName.ACTIVE_LOANS_SUMMARY_BY_BRANCH)
    public void callActiveLoansSummaryByBranch() {      

       final int result =  this.jdbcTemplate.update("call sp_ClientLoanStatusAsOnDate()");
       logger.info(ThreadLocalContextUtil.getTenant().getName() + ": Results affected by update:(ACTIVE_LOANS_SUMMARY_BY_BRANCH) " + result);
    }
