...
import org.apache.fineract.accounting.common.AccountingConstants.LOAN_INS_PARAMS;
...
import org.apache.fineract.portfolio.loanproduct.domain.InstallsRepository;
...
public class LoanProductWritePlatformServiceJpaRepositoryImpl implements LoanProductWritePlatformService {
    ...
    private final InstallsRepository installsRepository;
    ...
    public LoanProductWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,
            ...
            final ChargeRepositoryWrapper chargeRepository,final InstallsRepository installsRepository,
        ...
        this.installsRepository = installsRepository;
    ...
    public CommandProcessingResult createLoanProduct(final JsonCommand command) {
        ...
        try {
            ...
            final List<Installs> installments = splitInstalls(command);
            ...
            final LoanProduct loanproduct = LoanProduct.assembleFromJson(fund, loanTransactionProcessingStrategy, charges, installments, command,
                    this.aprCalculator, floatingRate);

            //            edits
            for(Installs ins:installments) {
            	ins.setLoanProduct(loanproduct);
            }
            loanproduct.setInstallments(installments);
            
//            setloan product for installment
//            set installment for loan product
...
//            split
            if (changes.containsKey("ins_data")) {
        		Gson gson = new Gson();
                final List<Installs> loan_amt = new ArrayList<>();

                if (command.parameterExists("ins_data")) {
                    final JsonArray insArray = command.arrayOfParameterNamed("ins_data");
                    if (insArray != null) {
                        for (int i = 0; i < insArray.size(); i++) {

                            final JsonObject Object = insArray.get(i).getAsJsonObject();
                            Installs InArray = gson.fromJson(Object, Installs.class);
                            loan_amt.add(InArray);  
//                            this.installsRepository.save(InArray);
                        }
                        
                    }
                }
//                product.setInstallments(loan_amt);
                for(Installs ins:loan_amt) {
                	ins.setLoanProduct(product);
                }
                product.setInstallments(loan_amt);
...
//	edits
	@Transactional
	private List<Installs> splitInstalls(final JsonCommand command) {
