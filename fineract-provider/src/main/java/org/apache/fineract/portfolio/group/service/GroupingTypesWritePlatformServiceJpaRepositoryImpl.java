...
public class GroupingTypesWritePlatformServiceJpaRepositoryImpl implements GroupingTypesWritePlatformService {
    ...
    @Autowired
    public GroupingTypesWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,  final RoutingDataSource dataSource,
...
//            Long test = newGroup.getId();
        	if (groupLevel.isCenter()) {
//              this.externalId = null;
//          	edits
          	try {
                  String officeExternalID = groupOffice.getExternalId();
                  long officeID = groupOffice.getId();
                  String[] splitExternalIDoffice = officeExternalID.split("\\.");
                 long externalCenterId = this.jdbcTemplate.queryForLong("select getCenterGroupCount(1,"+officeID+");");
                 String groupNumber = Long.toString(externalCenterId);
                 if(externalCenterId<10) {
                	 groupNumber = "0"+groupNumber;
                 }
                  combined = splitExternalIDoffice[1]+"."+splitExternalIDoffice[2]+"."+splitExternalIDoffice[3]+"."+groupNumber;
//                  +"."+Long.toString(externalCenterId);
                  String combinedCenter = combined+" (center)";
                  newGroup.setExternalId(combinedCenter);
        	...
        	else if(groupLevel.isGroup()) {
        		try {
        		if(parentGroup.getExternalId() != null) {
        			String ParentExternal =  parentGroup.getExternalId();
        			String[] externalCenter = ParentExternal.split("\\s+");
        			newGroup.setExternalId(externalCenter[0]);
        		}
        		}
        		catch(Exception e) {
...
//            edit by prashanth
            List<Group> GroupMembers = newGroup.getGroupMembers();
            for(Group group : GroupMembers)
            { 
            	try {
            		group.setExternalId(combined);
            	}
            	catch(Exception e)
            	{
            ...
            long count = 1;
            Set<Client> ClientMembers = newGroup.getClientMembers();
            for(Client client : ClientMembers)
            { 
            	try {                    
//            	String officeExternalID = groupOffice.getExternalId();
//                long officeID = groupOffice.getId();
                String tempString = parentGroup.getExternalId();
                String[] externalCenter = tempString.split("\\s+");
                String[] splitExternalIDGroup = externalCenter[0].split("\\.");
//               long externalClientCount = this.jdbcTemplate.queryForLong("select getClientGroupCount("+centerId+");");
                String clientCount = Long.toString(count);
                if(count < 10) {
                	clientCount = "0"+clientCount;
                }
               combinedClient = splitExternalIDGroup[2]+"."+splitExternalIDGroup[3]+"."+clientCount;
               
               client.setExternalId(combinedClient);
               count++;
            	}
            	catch(Exception e)
            	{
