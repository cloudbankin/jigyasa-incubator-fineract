...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService {
    ...
    public CommandProcessingResult createClient(final JsonCommand command) {
        ...
        try {
            ...
            if(newClient.getExternalId()==null && groupId != null ) {
            	String GroupExternalID = clientParentGroup.getExternalId();
            	Set<Client> count = clientParentGroup.getClientMembers();
            	long number = count.size() + 1;
            	String clientCounter = Long.toString(number);
            	if(number<10) {
            		clientCounter="0"+clientCounter;
            	}
            	
            	
            	String[] splitExternalIDgroup = GroupExternalID.split("\\.");
              
                 String combinedClientId = splitExternalIDgroup[2]+"."+splitExternalIDgroup[3]+"."+clientCounter;
//                 +"."+Long.toString(externalCenterId);
          	
                 newClient.setExternalId(combinedClientId);
//                 this.clientReadPlatformServiceImpl.retrieveClientCount(clientParentGroup.getClientMembers())
            }
...
//            edits
            String nomineeName = newClient.getLastname();
            long ClientID = newClient.getId();
            this.clientReadPlatformServiceImpl.insertIntoNomineeDetails(ClientID,nomineeName);
//            *********
